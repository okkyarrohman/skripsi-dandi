    ...
    <div class="w-full">
        <span class="card shadow text-decoration-none" style="display: flex; ">
            <div class="card-body" style="justify-content: left;">
                ...
                <form class="form-horizontal" action="{{ route('bom.update', ['id' => $boms->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <span>Nama Menu</span>
                    ...
                    <span>Jumlah</span>
                    <div class="input-group input-group-merge" style="max-width: 400px; margin-bottom: 10px;">
                        <input type="text" name="jumlah" class="form-control" value="{{$boms->jumlah}}" id="defaultFormControlInput" placeholder="Masukkan Jumlah" aria-describedby="defaultFormControlHelp">
                    </div>
